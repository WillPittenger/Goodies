...
public class AllGlobalOpts
{
	...
	public AllGlobalOpts(in AllGlobalOpts copyThis)
	{
		...
		WriteLinks = copyThis.WriteLinks;
	...
	{
		PyObject ConvertToPythonDict(in YtDlpWrapper ytWrapper);

		protected static PyObject ConvertListToPythonDict(in System.Collections.IEnumerable enumWhatToConvert, in YtDlpWrapper ytWrapper)
		{
				...
				foreach(System.Collections.DictionaryEntry kvpCur in  dict)
					if(kvpCur.Key is string strKey && kvpCur.Value is object objVal)
						pdResult[strKey] = objVal is System.Collections.IEnumerable enumChild
							? ConvertListToPythonDict(enumChild, ytWrapper)
							: objVal.ToPython();
	...
	public interface IOneOpt
	{
		...
		internal IReadOnlyDictionaryToOptLists? GetPythonParams(in YtDlpWrapper ytWrapper);
	}
	...
	public class OneOpt<Type>(in Type typeDef, in string strParamName, string? strPythonParmName = null)
		: IOneOpt
	{
		/// <summary>
		...
		public readonly string strPythonParamName = strPythonParmName ?? strParamName.Replace(@"-", string.Empty);
		...
		/// <seealso cref="CustomParamNameLookUp"/>
		/// <seealso cref="ParamListGenerator"/>
		public virtual IReadOnlyDictionaryToOptLists? GetPythonParams(in YtDlpWrapper ytWrapper)
		{
			...
			if(typeCurVal is bool bVal)
				return bVal
					? new System.Collections.Generic.Dictionary<string, System.Collections.Generic.IEnumerable<object>?>()
						{
							[strPythonParamName] =
								[
								true
			...
			if(typeCurVal == null)
				return new System.Collections.Generic.Dictionary<string, System.Collections.Generic.IEnumerable<object>?>(){ };
		...
		/// <summary>
		/// Lets you specify a method that generates the strings to use.  It should return either an empty dictionary or <see langword="null"/> if no parameters are needed.  This is commonly needed if multiple values are needed.  If it returns an entry with a null array, that entry will be ignored.
		/// </summary>
		public System.Func<OneOpt<Type>, IReadOnlyDictionaryToOptLists?>? PythonParamListGenerator
		{
			get;
	...
	/// </remarks>
	public class OneOptWithPrefix<Type>(in Type typeDef, in string strParamName, in string strValPrefix, in string? strPythonParmName = null)
		: OneOpt<Type>(typeDef, strParamName, strPythonParmName)
	{
		/// <summary>
		...
		/// <inheritdoc/>
		public override IReadOnlyDictionaryToOptLists? GetPythonParams(in YtDlpWrapper ytWrapper)
			=> IsDefaulted
	...
	public class ThreeWayOpt(in string strTrueParamName, in string strFalseParamName, in string? strTruePythonParamName = null, in string? strFalsePythonParamName = null)
		: IOneOpt
	{
		/// <summary>
		...
		/// </summary>
		public readonly string strPythonFlagName = strTruePythonParamName ?? strTrueParamName.Replace("-", string.Empty);
		...
		/// Returns a list of parameters that are needed.  If <see cref="IsDefaulted"/> is <see langword="true"/>, this will always be an empty array.  Otherwise, an array containing just either <see cref="strFalseParamName"/> or <see cref="strTrueParamName"/> will be returned.
		/// </summary>
		public IReadOnlyDictionaryToOptLists? GetPythonParams(in YtDlpWrapper ytWrapper)
			=> bVal switch
			{
				true or false
						=> new DictionaryToOptLists()
	...
	/// </remarks>
	public record UShortRangeOpt(in string strMinParam, in string strMaxParam, in string? strMinPythonParam = null, in string? strMaxPythonParam = null)
		: IOneOpt
	{
		/// <summary>
		...
		/// The parameter to use when <see cref="Min"/> is non-<see langword="null"/>.
		/// </summary>
		public readonly string strMinPythonParam = strMinPythonParam ?? strMinParam.Replace("-", string.Empty);

		/// <summary>
		/// The parameter to use when <see cref="Max"/> is non-<see langword="null"/>.
		/// </summary>
		public readonly string strMaxPythonParam = strMaxPythonParam ?? strMinParam.Replace("-", string.Empty);
	...
	public sealed class GeneralGroup
	{
		...
		{
			IgnoreErrors = copyThis.IgnoreErrors;
			AbortOnError = copyThis.AbortOnError;
			UseExtractors = copyThis.UseExtractors;
			DefSearchPrefix = copyThis.DefSearchPrefix;
			IgnoreConfigFiles = copyThis.IgnoreConfigFiles;
			NoConfigLoc = copyThis.NoConfigLoc;
			ConfigLoc = copyThis.ConfigLoc;
			AdditionalExtractorPlugInLocs = copyThis.AdditionalExtractorPlugInLocs;
			FlatPlayList = copyThis.FlatPlayList;
			LiveFromStart = copyThis.LiveFromStart;
			WaitForVid = copyThis.WaitForVid;
			MarkWatched = copyThis.MarkWatched;
			ColorWhenInStdOut = copyThis.ColorWhenInStdOut;
			ColorWhenInStdErr = copyThis.ColorWhenInStdErr;
		}
	...
	public sealed class NetGroup
	{
		...
		{
			ProxyURL = copyThis.ProxyURL;
			SocketTimeOut = copyThis.SocketTimeOut;
			SocketIpAddr = copyThis.SocketIpAddr;
			IpAddrMode = copyThis.IpAddrMode;
		}
	...
	public sealed class GeoRestrictionGroup
	{
		...
		{
			VerificationProxy = copyThis.VerificationProxy;
			XFF = copyThis.XFF;
		}
	...
	public sealed class VidSelGroup
	{
		...
		{
			MinFileSize = copyThis.MinFileSize;
			MaxFileSize = copyThis.MaxFileSize;
			ByDate = copyThis.ByDate;
			MatchFilters = copyThis.MatchFilters;
			BreakMatchFilters = copyThis.BreakMatchFilters;
			PlayListOrVidSel = copyThis.PlayListOrVidSel;
			AgeLimit = copyThis.AgeLimit;
			DownLoadArchive = copyThis.DownLoadArchive;
			BreakOnExisting = copyThis.BreakOnExisting;
			BreakPerInput = copyThis.BreakPerInput;
			SkipPlayListAfterErrors = copyThis.SkipPlayListAfterErrors;
		}
		...
		public class DateRelativeValWrapperValCore(DateRelativeValWrapperValCore.Origins Origin, ulong Distance, DateRelativeValWrapperValCore.DistanceScales Scale)
		: DateWrapperValCore
		{
			/// <summary>
	...
	public sealed class DownLoadsGroup
	{
		...
		{
			ConcurrentFragments = copyThis.ConcurrentFragments;
			LimitRate = copyThis.LimitRate;
			ThrottledRate = copyThis.ThrottledRate;
			MaxDownLoads = copyThis.MaxDownLoads;
			MaxRetries = copyThis.MaxRetries;
			RetrySleep = copyThis.RetrySleep;
			SkipUnavailableFragments = copyThis.SkipUnavailableFragments;
			KeepFragments = copyThis.KeepFragments;
			BufferSize = copyThis.BufferSize;
			ResizeBuffer = copyThis.ResizeBuffer;
			HttpChunkSize = copyThis.HttpChunkSize;
			PlayListOrder = copyThis.PlayListOrder;
			LazyPlayList = copyThis.LazyPlayList;
			XattrSetFileSizeAttr = copyThis.XattrSetFileSizeAttr;
			HlsUseMPEGTS = copyThis.HlsUseMPEGTS;
			Sections = copyThis.Sections;
	...
	public sealed class FileSysGroup
	{
		...
		public FileSysGroup(in FileSysGroup copyThis)
		{
			...
			PlaceHolderTextInGeneratedFileNames = copyThis.PlaceHolderTextInGeneratedFileNames;
			RestrictFileNames = copyThis.RestrictFileNames;
			UseWindowsFileNames = copyThis.UseWindowsFileNames;
			MaxFileNameLength = copyThis.MaxFileNameLength;
			WhenToAllowOverWrites = copyThis.WhenToAllowOverWrites;
			ResumePartialDownLoads = copyThis.ResumePartialDownLoads;
			UsePartFiles = copyThis.UsePartFiles;
			MTime = copyThis.MTime;
			DescShouldBeWritten = copyThis.DescShouldBeWritten;
			InfoJsonShouldBeWritten = copyThis.InfoJsonShouldBeWritten;
			PlayListMetaFilesShouldBeWritten = copyThis.PlayListMetaFilesShouldBeWritten;
			CommentsShouldBeWritten = copyThis.CommentsShouldBeWritten;
			CookiesFromFile = copyThis.CookiesFromFile;
			CookiesFromBrowser = copyThis.CookiesFromBrowser;
			CacheDir = copyThis.CacheDir;
		}
		...
		public sealed class PathsGroup
		{
			...
			public PathsGroup(in PathsGroup copyThis)
			{
				...
				Temp = copyThis.Temp;
				Subs = copyThis.Subs;
				Thumbs = copyThis.Thumbs;
				Desc = copyThis.Desc;
				InfoJSON = copyThis.InfoJSON;
				Links = copyThis.Links;
				PlayListThumb = copyThis.PlayListThumb;
				PlayListDesc = copyThis.PlayListDesc;
				PlayListThumb = copyThis.PlayListThumb;
				PlayListInfoJSON = copyThis.PlayListInfoJSON;
				Chapters = copyThis.Chapters;
				PlayListEntries = copyThis.PlayListEntries;
			}
		...
		public sealed class TemplatesForOutputFilesGroup
		{
			...
			public TemplatesForOutputFilesGroup(in TemplatesForOutputFilesGroup copyThis)
			{
				...
				InfoJSON = copyThis.InfoJSON;
				Links = copyThis.Links;
				PlayListThumbs = copyThis.PlayListThumbs;
				PlayListDesc = copyThis.PlayListDesc;
				PlayListInfoJSON = copyThis.PlayListInfoJSON;
				Chapters = copyThis.Chapters;
				PlayListEntries = copyThis.PlayListEntries;
	...
	{
		public VerbosityAndSimulationGroup()
		{
		}

		public VerbosityAndSimulationGroup(in VerbosityAndSimulationGroup copyThis)
		{
					...
					: new DictionaryToOptLists()
						{
							[@"--print-to-file"] =
							[
								$"{strPrefix}:{strTemplate}", fileDest.FullName,
							],
						};
		}
	...
	public sealed class WorkAroundsGroup
	{
		...
		{
			Encoding = copyThis.Encoding;
			ExtraHdrs = copyThis.ExtraHdrs;
			BIDI = copyThis.BIDI;
			SleepRequests = copyThis.SleepRequests;
			SleepIntervals = copyThis.SleepIntervals;
			SubsSleep = copyThis.SubsSleep;
		}
	...
	public sealed class VidFmtGroup
	{
		...
		{
			Sorting = copyThis.Sorting;
			ForceSorting = copyThis.ForceSorting;
			VidMultiStreamsAllowed = copyThis.VidMultiStreamsAllowed;
			AudioMultiStreamsAllowed = copyThis.AudioMultiStreamsAllowed;
			PreferFree = copyThis.PreferFree;
			Check = copyThis.Check;
	...
	public sealed class SubsGroup
	{
		...
		{
			WriteFile = copyThis.WriteFile;
			WriteAutoFile = copyThis.WriteAutoFile;
			FmtForFiles = copyThis.FmtForFiles;
			LangsToDownLoad = copyThis.LangsToDownLoad;
		}
	...
	public sealed class AuthenticationGroup
	{
		...
		public AuthenticationGroup(in AuthenticationGroup copyThis)
		{
			UserNameAndPwd = copyThis.UserNameAndPwd;
			TwoFactor = copyThis.TwoFactor;
			NetRC = copyThis.NetRC;
			VidSpecificPwd = copyThis.VidSpecificPwd;
		}
...
#pragma warning restore IDE0046 // Convert to conditional expression
			...
			public IReadOnlyDictionaryToOptLists GetPythonParams(in YtDlpWrapper ytWrapper)
			{
				return UserName == string.Empty && Pwd != string.Empty
					?          throw new System.InvalidOperationException("If you specify a password, you must specify a user name.")
					: Pwd == string.Empty && UserName != string.Empty
					? throw new System.InvalidOperationException("If you specify a user name, you must specify a password.")
					: (IReadOnlyDictionaryToOptLists)new DictionaryToOptLists()
					{
						[@"username"] = new System.Collections.Generic.List<string>()
							{
								UserName,
							},
						[@"password"] = new System.Collections.Generic.List<string>()
							{
								Pwd
							},
					};
		...
		/// </summary>
		[YtDlpOptDoc(nameof(Rsrcs.strAuthenticationUserNameAndPwd), @"Specifies how to log in with user name and password.  Note: If you use a user name, you must provide a password and vice-versa.  Support depends on the website.", typeof(AuthenticationGroup), @"--username", @"--password")]
		...
		/// </summary>
		[YtDlpOptDoc(nameof(Rsrcs.strAuthenticationTwoFactor), @"Two-factor authentication code", typeof(AuthenticationGroup), @"--twofactor")]
		...
		/// </summary>
		[YtDlpOptDoc(nameof(Rsrcs.strAuthenticationNetRC), @"Use .netrc authentication data", typeof(AuthenticationGroup), @"--netrc")]
		...
		/// </summary>
		[YtDlpOptDoc(nameof(Rsrcs.strAuthenticationVidSpecificPwd), @"Video-specific password", typeof(AuthenticationGroup), @"--video-password")]
	...
	public sealed class PostProcessingGroup
	{
		...
		{
			ExtractAudioToFmt = copyThis.ExtractAudioToFmt;
			AudioQuality = copyThis.AudioQuality;
			RemuxVid = copyThis.RemuxVid;
			RecodeVid = copyThis.RecodeVid;
			PostProcessorArgs = copyThis.PostProcessorArgs;
			KeepVid = copyThis.KeepVid;
			OverWrites = copyThis.OverWrites;
			...
			WriteMetaDataToXATTR = copyThis.WriteMetaDataToXATTR;
			FixUpPolicy = copyThis.FixUpPolicy;
			ExecCmd = new(copyThis.ExecCmd);
			ConvertSubs = copyThis.ConvertSubs;
			ConvertThumbs = copyThis.ConvertThumbs;
			RemoveChapters = copyThis.RemoveChapters;
			ForceKeyFramesAtCuts = copyThis.ForceKeyFramesAtCuts;
		}
		...
		/// </summary>
		public sealed record ParseMetaDataInstructOpt(in string strPrefix) : IOneOpt, IPythonTypeMaker
		{
			/// <summary>
			...
			public System.Collections.Generic.IEnumerable<string> Params
				=> strFrom == string.Empty || strTo == string.Empty
					? []
					: [@"--replace-in-metadata", $"{strPrefix}:{strFrom}:{strTo}"];
			...
			public IReadOnlyDictionaryToOptLists GetPythonParams(in YtDlpWrapper ytWrapper)
			{
				if(ytWrapper.yt is not null)
				{
					using Py.GILState lockInfo = Py.GIL();

					return strFrom == string.Empty || strTo == string.Empty
						? []
						: new DictionaryToOptLists()
							{
								[@"postprocessors"] =
									[
										...
										{
											[@"key"] = ytWrapper.yt.postprocessor.metadataparser.MetadataParserPP.interpretter,
											[@"when"] = strPrefix,
			...
			public PyObject ConvertToPythonDict(in YtDlpWrapper ytWrapper)
				=> IPythonTypeMaker.ConvertListToPythonDict(GetPythonParams(ytWrapper), ytWrapper);
		}
		...
		public sealed class ReplaceInMetaDataInstructOpt(in string strPrefix) : IOneOpt, IPythonTypeMaker
		{
			/// <summary>
			/// The stage at which replacement will happen.
			/// </summary>
			...
			public IReadOnlyDictionaryToOptLists GetPythonParams(in YtDlpWrapper ytWrapper)
			{
				if(ytWrapper.yt is not null)
				{
					...
					return strsetFields.Count <= 0 || regex == null || strReplace == string.Empty
						? []
						: new DictionaryToOptLists()
							{
								[@"postprocessors"] =
									[
										new System.Collections.Generic.Dictionary<string, object>()
										{
											[@"key"] = ytWrapper.yt.postprocessor.metadataparser.MetadataParserPP.replacer,
											[@"when"] = strPrefix,
											[@"actions"] = new System.Collections.Generic.List<object>()
											{
												new System.Collections.Generic.List<object>()
		...
		/// <param name="Args">The arguments to send</param>
		/// <param name="When">When to trigger the processor.  Use a value from <see cref="WhenValsForPostProcessing"/>.  You can pass <see langword="null"/> here.  That’s treated as <see cref="WhenValsForPostProcessing.all"/>.</param>
		public sealed record PostProcessorArgsForExeVal(in System.IO.FileInfo fileExe, in System.Collections.Generic.IReadOnlyDictionary<string, string>? Args = null, in WhenValsForPostProcessing? When = null) : BasePostProcessorArgsVal(Args, When)
		{
			/// <summary>
		...
		/// <param name="When">When to trigger the processor.  Use a value from <see cref="WhenValsForPostProcessing"/>.  You can pass <see langword="null"/> here.  That’s treated as <see cref="WhenValsForPostProcessing.all"/>.</param>
		public sealed record NamedExeProcessorArgsVal(in string strExeFileNameWithOutPath, in System.Collections.Generic.IReadOnlyDictionary<string, string>? Args = null, in WhenValsForPostProcessing? When = null) : BasePostProcessorArgsVal(Args, When)
		{
			/// <summary>
	...
	{
		public SponsorBlockGroup()
		{
		}

		public SponsorBlockGroup(in SponsorBlockGroup copyThis)
		{
			MarkCats = copyThis.MarkCats;
			ChapterTitle = copyThis.ChapterTitle;
			Disable = copyThis.Disable;
			ApiURL = copyThis.ApiURL;
		}
	...
	public sealed class ExtractorGroup
	{
		...
		{
			Retries = copyThis.Retries;
			AllowDynamicMPD = copyThis.AllowDynamicMPD;
			SplitHlsDiscontinuities = copyThis.SplitHlsDiscontinuities;
			Args = copyThis.Args;
		}
	...
	/// </summary>
	public VerbosityAndSimulationGroup VerbosityAndSimulation
	{
		get;
	...
	/// </summary>
	public SponsorBlockGroup SponsorBlock
	{
		get;
	...
	private static IReadOnlyDictionaryToOptLists MergeDictionaries(params IReadOnlyDictionaryToOptLists[] valuesToMerge)
	{
		DictionaryToOptLists mapResult = [];

		foreach(IReadOnlyDictionaryToOptLists mapCurVal in valuesToMerge)
			foreach(System.Collections.Generic.KeyValuePair<string, IOptList?> kvpCur in mapCurVal)
			{
				System.Collections.Generic.LinkedList<object> llistValues = [];

				if(kvpCur.Value != null)
					foreach(object objCurVal in kvpCur.Value)
						llistValues.AddLast(objCurVal);

				mapResult[kvpCur.Key] = mapResult.TryGetValue(kvpCur.Key, out IOptList? value)
					? value is IOptList realList
						? realList.Concat(llistValues)
						: llistValues
					: llistValues;
			}
	...
	internal PyDict ToPythonDict(YtDlpWrapper ytWrapper)
	{
		if(allOpts is System.Collections.Generic.IEnumerable<IOneOpt> allRealOpts)
		{
			using Py.GILState lockInfo = Py.GIL();

			PyDict pdResult = new();

			IReadOnlyDictionaryToOptLists mapMerged = MergeDictionaries(
				[
					..allRealOpts.Select
						(curOpt
							=> curOpt.GetPythonParams(ytWrapper) ?? new DictionaryToOptLists(){ }
						)
					]
				);

			foreach(System.Collections.Generic.KeyValuePair<string, IOptList?> kvpCur in mapMerged)
				pdResult.SetItem(kvpCur.Key, kvpCur.Value is IPythonTypeMaker maker
					? maker.ConvertToPythonDict(ytWrapper)
