...
/// </summary>
/// <remarks>
/// <para>While it references videos in the name, it actually can accept channels and playlists too.  Unlike <see cref="GetInfoFromYtDlp" />, <see cref="GetVidTimeSpan" /> gets data sooner as yt-dlp can send the filename up the pipeline immediately rather than waiting for the JSON to be built.  That doesn’t happen until yt-dlp has completely processed that parameter.  So for a playlist, each entry in the playlist can be sent up the pipeline immediately while <see cref="GetInfoFromYtDlp" /> would need to wait for the playlist to be complete.  <see cref="GetVidTimeSpan"/> returns all results as instances of <see cref="System.TimeSpan"/>.  The items can be of type <see cref="string"/>, <see cref="System.Uri"/>, or <see cref="System.IO.FileInfo"/>.  If a item is a <see cref="string"/>, its checked to see if it looks like a URL of the HTTP/HTTPS protocol.  If so, that URL is passed to yt-dlp and the duration obtained from there.  Otherwise, the string is checked to see if it’s an existing file on the local file system with <see cref="System.IO.File.Exists(string?)"/>.  In that case, yt-dlp won’t be used to obtain the length of the video.  Instead, <see cref="GetVidTimeSpan"/> tries to use COM to get the video length.</para>
/// </remarks>
